...
class ImageSubscriber(Node):
    def __init__(self):
        ...
        self.right_zed_left_sub = Subscriber(
            self,
            Image,
            '/right_zed/zed_node/left/image_rect_color'
        )
        self.right_zed_right_sub = Subscriber(
            self,
            Image,
            '/right_zed/zed_node/right/image_rect_color'
        )
        self.right_rgb_pub = self.create_publisher(Image,'tri_right_zed_cropped',10)
        self.right_depth_pub = self.create_publisher(Image,'tri_right_zed_depth',10)
        self.right_camera_info_sub = self.create_subscription(CameraInfo,'/right_zed/zed_node/left/camera_info',self.rightCameraInfoCallback,10)
        self.right_intrinsics_ = None
        self.right_synchronizer = ApproximateTimeSynchronizer(
            [self.right_zed_left_sub, self.right_zed_right_sub],
            queue_size=10,
            slop=0.1
        )
        self.right_synchronizer.registerCallback(self.rightCallback)
