...
class MinimalPublisher(Node):
    ...
    def __init__(self):
        ...
        self.right_zed_left_image_publisher_ = self.create_publisher(CompressedImage, '/repub_right_zed_left_color/compressed', 10)
        self.right_zed_depth_image_publisher_ = self.create_publisher(Image,'/repub_right_zed_depth',10)
        self.cv_bridge_ = CvBridge()
        file_location = os.path.dirname(os.path.realpath(__file__))
        d455_folder = '/home/kushtimusprime/2024_03_12_bags/alienware_sensitivity_bags/dense_kitchen_fig8_zed'
        left_zed_image_list = sorted(glob.glob(os.path.join(d455_folder, 'left_tri_rgb', '*.png')))[::1]
        left_zed_depth_list = sorted(glob.glob(os.path.join(d455_folder, 'left_tri_depth', '*.npy')))[::1]
        right_zed_image_list = sorted(glob.glob(os.path.join(d455_folder, 'right_tri_rgb', '*.png')))[::1]
        right_zed_depth_list = sorted(glob.glob(os.path.join(d455_folder, 'right_tri_depth', '*.npy')))[::1]
        realsense_image_list = sorted(glob.glob(os.path.join(d455_folder, 'camera_color', '*.png')))[::1]
        realsense_depth_list = sorted(glob.glob(os.path.join(d455_folder, 'camera_depth', '*.npy')))[::1]
        total_image_stream = []
        for t, (left_zed_image_file, left_zed_depth_file,right_zed_image_file,right_zed_depth_file,realsense_image_file,realsense_depth_file) in enumerate(zip(left_zed_image_list, left_zed_depth_list,right_zed_image_list, right_zed_depth_list,realsense_image_list,realsense_depth_list)):
            left_zed_image = cv2.imread(left_zed_image_file)
            right_zed_image = cv2.imread(right_zed_image_file)
            realsense_image = cv2.imread(realsense_image_file)
            
            left_zed_depth_image = np.load(left_zed_depth_file)
            right_zed_depth_image = np.load(right_zed_depth_file)
